...
(sleep 5; echo yes; sleep 2; echo n;...) | SSHPASS='falcon' sshpass -e ssh -tt -o StrictHostKeyChecking=no fpp@FPP ssh -R 80:localhost:80 ssh.localhost.run & disown
rm -f /home/fpp/media/plugins/remote-falcon/remote_url.txt
sleep 5
SSHPASS='falcon' sshpass -e ssh -tt -o StrictHostKeyChecking=no fpp@FPP ssh -R 80:localhost:80 ssh.localhost.run > /home/fpp/media/plugins/remote-falcon/remote_url.txt 2>&1 & disown
sleep 5
